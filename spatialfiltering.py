...
def SpatialFiltering(depvar, indepvars, spatiallagvars, data, nb, style="d", zeropolicy=False, tol=0.1, zerovalue=0.0001, ExactEV=False, symmetric=True, alpha=None, alternative="two.sided",verbose=False):
    ...
    #supplement given neighbors list with spatial weights for given coding scheme (r=row-standardized, d=double standardized, b=binary, v=variance stabilized)
    w=pysal.open(nb).read()
    w.transform = style

    S,ids = pysal.full(w)

    # if symmetric=true, constructs a weights list object corresponding to the sparse matrix 1/2 (W + W')
    if symmetric:
        S = 0.5 * (S + S.T)
    ...
    if(xsar.shape[0] <> S.shape[0]):
        raise Exception("Input data and neighbourhood list have different dimensions")
    ...
    y.shape = (y.shape[0], 1)
    if(np.count_nonzero(np.isnan(pysal.spreg.OLS(np.array(y),np.array(X[:,1:])).betas)) > 0):
    ...
    M = np.identity(nofreg) - X * LA.solve((np.transpose(X) * X),np.transpose(X))
    MSM = M * S * M
    ...
    if verbose:
        print "Step", Aout[0,0], "SelEvec", Aout[0,1], "MinMi", Aout[0,3], "ZMinMi", Aout[0,4],"Pr(ZI)", Aout[0,5]
    ...
    sel[:,2] = (v > abs(zerovalue)).astype(int) - (v < -abs(zerovalue)).astype(int)
    ...
    # Due to orthogonality each coefficient can be estimate individually
        for j in range(0,nofreg):
            if (sel[j,2] == acsign): #Use only feasible unselected evecs
                gamma4eigenvec[j,1] = LA.solve(np.transpose(d[:,j])*d[:,j],np.transpose(d[:,j])*y)
    ...
    for i in range(0,nofreg): #Outer Loop
        ...
        for j in range(0,nofreg): #Inner Loop - Find next eigenvector
            if( sel[j,2] == acsign ): #Use only feasible unselected evecs
                ...
                if onlysar:
                    ...
                    res = y - xe * LA.solve(np.transpose(xe)*xe,np.transpose(xe)*y)

                mi = ((np.transpose(res) * S) * res ) / (np.transpose(res) * res)
                if ExactEV:
                    M = np.identity(nofreg) - xe * LA.solve(np.transpose(xe)*xe,np.transpose(xe))
        ...
        if( idx > 0) :
            ...
            out = [i+1,idx,v[idx-1],MinMi[0,0],ZMinMi[0,0],altfunction(ZMinMi,alternative)[0,0],(1 - ((np.transpose(y)*M)*y/TSS))]
            if verbose:
                print "Step", out[0], "SelEvec", out[1], "MinMi", out[3], "ZMinMi", out[4],"Pr(ZI)", out[5]
            ...
            if not alpha:
                if(abs(ZMinMi) < tol):
                    ...
                    if not ExactEV:
                        print "An inversion has been detected. The procedure will terminate now.\n"
                        print "It is suggested to use the exact expectation and variance of Moran's I\n"
                        print "by setting the option ExactEV to TRUE.\n"
                    break
            else:
...
##    indepvars = ["INC", "HOVAL"]
##    spatiallag = True
##    nb = "C:\\Program Files\\R\\R-3.1.3\\library\\spdep\\etc\\weights\\columbus.gal"
##    data = "C:\\columbus.dbf"
##    style = "r"
##    zeropolicy = False
##    tol = 0.1
...
##    verbose = True
    ...
    alternative = "two.sided"
    verbose = True
    SpatialFiltering(depvar, indepvars, spatiallag, data, nb, style, zeropolicy, tol, zerovalue, ExactEV, symmetric, alpha, alternative, verbose)